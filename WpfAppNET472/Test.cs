...
namespace WpfAppNET472
{
    ...
    {
        public static IEnumerable<byte[]> SpliteVideoFrameToRGB32(this string filename)
        {
            ...
            DirectN.Functions.MFCreateSourceReaderFromURL(filename, attribute, out source);
            ...
            using (var timestamp_ptr = new ComMemory(Marshal.SizeOf<uint>()))
            {
                while(true)
                {
                    ...
                    yield return bb;
                    //yield return BitmapSource.Create((int)w, (int)h, 96, 96, PixelFormats.Bgr32, null, bb, (int)w * 4);
